...
- `BOT_TOKEN`
- `MANAGER_USERNAME`
- `ADMIN_CHAT_ID` — один чат менеджеров (для обратной совместимости).
- `ADMIN_CHAT_IDS` — несколько чатов менеджеров через запятую, например `123456789,-1001234567890`. Все уведомления о заказах отправляются во все указанные чаты. Если задан и `ADMIN_CHAT_ID`, и `ADMIN_CHAT_IDS`, основной `ADMIN_CHAT_ID` автоматически добавляется в список, если его там нет.
